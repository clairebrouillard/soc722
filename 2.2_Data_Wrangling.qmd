...
flights |>
  arrange(distance)
```
Flights from JFK to HNL travel the farthest distance. The flight that traveled the shortest distance was from EWR to LGA.
...
## 7
The original doesn't work because only tailnum has been selected so the arrange function cannot find arr_delay. To fix it, you'd have to include arr_delay in the original selection.
```{r}
# flights |> 
#  select(tailnum) |> 
#  arrange(arr_delay)
...
## 1
It's incredibly difficult to completely disentangle the effects of bad airports vs. bad carriers; however, comparing the worst 5 average arrival delays for each carrier is useful because it allows for manual comparison of destinations, which shows that certain destinations (ATL, PHL, PHX, etc.) are among the worst delays for multiple carriers, suggesting that perhaps there is an issue with these airports.
```{r}
flights |>
...
Without controlling for bad airports, UA is the carrier with the worst average delays.
```{r}
flights |>
  group_by(carrier, dest) |>
  select(carrier, dest, arr_delay) |>
...
## 5
count() allows you to count the number of occurrences of any given variable or combination of variables. The sort = TRUE argument arranges the number of occurrences in descending order.
```{r}
flights |>
  filter(carrier == "UA") |>
